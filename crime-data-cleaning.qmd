---
title: "DSAN 5100 Project. Crime Data Cleaning"
...
agg_crime_oh <- read_csv("crime-data/crime-numbers/Aggravated-assault-Ohio.csv")
agg_crime_ut <- read_csv("crime-data/crime-numbers/Aggravated-assault-Utah.csv")
...
hom_crime_oh <- read_csv("crime-data/crime-numbers/Homicide-Ohio.csv")
hom_crime_ut <- read_csv("crime-data/crime-numbers/Homicide-Utah.csv")
...
rp_crime_oh <- read_csv("crime-data/crime-numbers/Rape-Ohio.csv")
rp_crime_ut <- read_csv("crime-data/crime-numbers/Rape-Utah.csv")
...
rob_crime_oh <- read_csv("crime-data/crime-numbers/Robbery-Ohio.csv")
rob_crime_ut <- read_csv("crime-data/crime-numbers/Robbery-Utah.csv")
rob_crime_us <- read_csv("crime-data/crime-numbers/Robbery-US.csv")
```


```{r}
list_df_agg <- list(agg_crime_ca, agg_crime_fl, agg_crime_ny, agg_crime_mi, agg_crime_oh, agg_crime_ut, agg_crime_us)
...
```{r}
list_df_hom <- list(hom_crime_ca, hom_crime_fl, hom_crime_ny, hom_crime_mi, hom_crime_oh, hom_crime_ut, hom_crime_us)
...
list_df_rp <- list(rp_crime_ca, rp_crime_fl, rp_crime_ny, rp_crime_mi, rp_crime_oh, rp_crime_ut, rp_crime_us)
...
list_df_rob <- list(rob_crime_ca, rob_crime_fl, rob_crime_ny, rob_crime_mi, rob_crime_oh, rob_crime_ut, rob_crime_us)
...
```{r}
# Include a total "violent_crime_count" for a total
...
```{r}
# Group by state and year to get a yearly number of crimes

crime_df_with_pop_annual <- crime_df_with_pop |>
        group_by(State, Year) |>
            summarize(
                aggravated_assault_count = sum(aggravated_assault_count),
                homicide_count = sum(homicide_count),
                rape_count = sum(rape_count),
                robbery_count = sum(robbery_count),
                violent_crime_count = sum(violent_crime_count),
...
# Add rates for YEARLY crime data using population data

crime_df_with_pop_annual <- crime_df_with_pop_annual |>
                    mutate(
...
# Add rates for MONTHLY crime data using population data

crime_df_with_pop_monthly <- crime_df_with_pop |>
                    mutate(
                        aggravated_assault_rate = (aggravated_assault_count / population_count)*100000,
                        homicide_rate = (homicide_count / population_count)*100000,
                        rape_rate = (rape_count / population_count)*100000,
...
write_csv(crime_df_with_pop_monthly, "data/processed-data/crime_data_monthly_clean.csv")

write_csv(crime_df_with_pop_annual, "data/processed-data/crime_data_yearly_clean.csv")
