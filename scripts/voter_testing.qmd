...
knitr::kable(summary_stats, digits = 4, caption = "Descriptive Statistics by State Category")
```
...
## Test 2: Independent Samples T-Tests (Pairwise Comparisons)
...
### Chi-Square 3a: Election Type vs State Category
```{r chisq-election}
contingency_election <- table(df$election_type, df$state_color)
...
summary_results <- tibble(
    Test = c(
        ...
        "Election x State", "Turnout x State"
    ),
    Statistic = c(
    ...
    p_value = round(c(
        ...
        ttest_bp$p.value, chisq_election$p.value, chisq_turnout$p.value
    ), 6),
    Decision = c(
